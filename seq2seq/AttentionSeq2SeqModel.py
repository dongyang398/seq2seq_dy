...
import time
'''
attention的seq2seq模型，序列到序列，使用encoder和decoder均使用rnn
...
class AttentionSeq2SeqModel(object):
    ...
    def __init__(self, model_path=None,
        ...
        # 2检测训练参数
        if src_vocab_size == None \
                or tgt_time_step == None \
                or tgt_vocab_size == None \
                or start_token_id == None or \
                end_toekn_id == None:
    ...
    def build_input_tensor(self, src_embedding_size, tgt_embedding_size):
        with tf.name_scope("input_tensor"):
            ...
            # 返回的embedded_X.shape = [time_step,batch_size,embedding_size]
            ...
            self.tgt_embedding = tf.get_variable(shape=[self.tgt_vocab_size,
                                                        tgt_embedding_size],
            ...
            # 保存当前batch的最长序列值，mask的时候需要用到：计算loss时忽略pad的部分，这部分的loss不需要算
    ...
    def build_input_tensor_inference(self):
        ...
        with tf.variable_scope("input_tensor_inference"):
            ...
            self.start_tokens = tf.tile(tf.constant(value=self.start_token_id, dtype=tf.int32, shape=[1]),
                                        multiples=self.inference_batch_size,
                                        name="start_tokens")
    ...
    def build_encoder(self, encoder_rnn_layer_size,
                      encoder_num_units,
                      encoder_cell_type="LSTM"):
        # 定义encoder的rnn_layer
        with tf.name_scope("encoder_rnn"):
            rnn_layer = self.get_rnn_layer(encoder_rnn_layer_size,
                                           encoder_num_units,
                                           encoder_cell_type)
            # 将rnn沿时间序列展开
            ...
            encoder_outputs, encoder_states = tf.nn.dynamic_rnn(rnn_layer,
    ...
    # 双向rnn的encoder参照Google官方的nmt教程
    def build_encoder_bi(self, encoder_rnn_layer_size,
                         encoder_num_units,
                         encoder_cell_type="LSTM"):
        ...
        with tf.name_scope("encoder"):
            ...
            '''                    
                    ...
                    有2个解决办法，将encoder rnn的num_units变成decoder的一半或者反过来将decoder rnn的num_units增大一倍
            '''
    ...
    # 构造训练时的decoder
    def build_decoder(self, encoder_states, encoder_outputs, decoder_rnn_layer_size, decoder_num_units,
        ...
        with tf.variable_scope("decoder"):
            ...
            # 加入attention
            self.decoder_rnn_layer_attention = self.build_rnn_layer_attention(encoder_outputs,
                                                                              self.decoder_rnn_layer,
                                                                              decoder_num_units,
                                                                              self.src_batch_seq_len)
            ...
            # 这里projection_layer不指定激活函数为softmax，最后构建loss的传入的是logits，logits是没有经过激活函数的
            # 的值，logits = W*X+b
            self.projection_layer = tf.layers.Dense(units=self.tgt_vocab_size,
                                                    kernel_initializer=tf.truncated_normal_initializer(mean=0.0,
                                                                                                       stddev=0.1))
            # 训练使用的training_helper
            # tf.contrib.seq2seq.TrainingHelper Decoder的一个类，只能在训练时使用，作用是读取输入。
            ...
            decoder = tf.contrib.seq2seq.BasicDecoder(
                                        self.decoder_rnn_layer_attention,
                                        training_helper, init_state,
    ...
    # 构造decoder的前向网络
    def build_decoder_inference(self, encoder_outputs,
                                encoder_states,
                                beam_width,
                                decoder_num_units,
                                decoder_rnn_layer_size,
                                decoder_cell_type,
                                decoding_method):
        '''
        ...
        :param beam_width:
        :param decoder_num_units:
        :param decoder_rnn_layer_size:
        :param decoder_cell_type:
        :param decoding_method: ecoding_method="greedy"
        :return:
        '''
        ...
        with tf.variable_scope("decoder"):
            ...
            # greedy解码
                ...
                rnn_layer_attention = self.build_rnn_layer_attention(encoder_outputs,
                                                                     rnn_layer,
                                                                     decoder_num_units,
                                                                     self.src_batch_seq_len)
                predicting_helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(self.tgt_embedding,
                                                                             self.start_tokens,
                                                                             self.end_token_id)
                ...
                raise Exception('不支持的解码方法，只可选 greedy 或者 beam ')
        # Dynamic decoding
        predicting_decoder_output, _, _ = tf.contrib.seq2seq.dynamic_decode(decoder,
                                                                            output_time_major=False,
                                                                            maximum_iterations=self.tgt_time_step,
                                                                            swap_memory=True)
        # predicted_ids：返回shape:[batchsize,max_len,beam_width]
        # tf.identity()就是返回一个一模一样的新的tensor
        if decoding_method == 'beamsearch':
    ...
    def seq_loss(self, rnn_output):
        ...
        loss_mask = tf.sequence_mask(self.tgt_batch_seq_len,
                                     self.tgt_time_step,
                                     dtype=tf.float32,
                                     name="masks")
    ...
    # 构建训练计算图和推理计算图 解决attention参数共享问题
    def build_graph(self,
        ...
        # 先构建推理图，后面创建的input tensor覆盖inference的，因为训练中需要用到train graph中的图
        with inference_graph.as_default():
            ...
            if is_encoder_bidirectional:
                # 需要注意使用bidirectional，encoder rnn的num_units变为decoder的一半，
                # 这是为了能够保证encoder_states和decoder的输入shape能对应上
                encoder_outputs, encoder_states = self.build_encoder_bi(rnn_layer_size,
                                                                        rnn_num_units/2,
                                                                        cell_type)
            else:
                encoder_outputs, encoder_states = self.build_encoder(rnn_layer_size,
                                                                     rnn_num_units,
                                                                     cell_type)
            # 选择合适的解码方法
            self.build_decoder_inference(encoder_outputs,
                                         encoder_states,
                                         beam_width,
                                         rnn_num_units,
                                         rnn_layer_size,
            ...
            with train_graph.as_default():
                ...
                if is_encoder_bidirectional:
                    ...
                    encoder_outputs, encoder_states = self.build_encoder(rnn_layer_size,
                                                                         rnn_num_units,
                                                                         cell_type)
                decoder_outputs = self.build_decoder(encoder_states,
                                                     encoder_outputs,
                                                     rnn_layer_size,
                                                     rnn_num_units,
                                                     cell_type)
                ...
                # mask的作用：计算loss时忽略pad的部分，这部分的loss不需要算，
                masks = tf.sequence_mask(self.tgt_batch_seq_len, self.tgt_batch_max_len, dtype=tf.float32, name='masks')
                with tf.name_scope("optimization"):
                    # loss
                    self.loss = tf.contrib.seq2seq.sequence_loss(training_logits,
                                                                 self.tgt_batch_y,
                                                                 masks)
                    optimizer = tf.train.AdamOptimizer(lr)
                    # 梯度下降
                    gradients = optimizer.compute_gradients(self.loss)
                    # tf.clip_by_value() 功能：可以将一个张量中的数值限制在一个范围之内
    ...
    # 训练
    def train(self, model_path, generator, src_embedding_size,
              tgt_embedding_size, is_encoder_bidirectional=False,
              rnn_layer_size=2, rnn_num_units=128, cell_type="LSTM",
              lr=0.001, decoding_method="greedy", beam_width=10):
        train_graph, inference_graph = self.build_graph(src_embedding_size,
                                                        tgt_embedding_size,
        ...
        with tf.Session(graph=train_graph) as sess:
            ...
            # writer.close()
            # 获取数据生成器
            for src_batch, tgt_batch_x, tgt_batch_y, \
                tgt_batch_max_len, src_batch_seq_len, \
                tgt_batch_seq_len, batch_num, epoch_num in generator:
                b_loss, _ = sess.run([self.loss, self.train_op],
                                     feed_dict={
                                        self.src_batch: src_batch,
                                        self.tgt_batch_x: tgt_batch_x,
                                        self.tgt_batch_y: tgt_batch_y,
    ...
    def predict(self, input, input_seq_len):
        ...
        answer_logits = self.sess.run(logits,
                                      {source_batch_input: input,
