...
'''
基本的seq2seq模型，序列到序列，使用encoder和decoder均使用LSTM，没有attention
...
class BasicSeq2SeqModel(object):
    ...
    def __init__(self, model_path=None, *,
        ...
        if model_path != None:
            ...
            return
        # 检测训练参数
        if src_vocab_size == None or \
                tgt_time_step == None or \
                tgt_vocab_size == None or \
                start_token_id == None or \
                end_toekn_id == None:
    ...
    def build_input_tensor(self, src_embedding_size, tgt_embedding_size):
        with tf.name_scope("input_tensor"):
            ...
            self.tgt_embedding = tf.get_variable(shape=[self.tgt_vocab_size, tgt_embedding_size],
    ...
    def build_encoder_bi(self, encoder_rnn_layer_size,
        ...
        with tf.name_scope("encoder_rnn"):
            ...
            bi_state的结构:(fw_state,bw_state)
                                ...
                                bw_state = ((c,h),(c,h)...)
            '''
            bi_outputs, bi_state = tf.nn.bidirectional_dynamic_rnn(fw_rnn_layer,
                                                                   bw_rnn_layer,
                                                                   self.embedded_src_batch,
                                                                   sequence_length=self.src_batch_seq_len,
                                                                   time_major=False,
                                                                   dtype=tf.float32)
            '''                    
            将前向rnn和后向rnn的output的最后一个维度连接起来,比如 fw:[128,10,100],bw:[128,10,100],
            那么连接后为[128,10,200]这样导致一个问题就是encoder rnn的输出和decoder rnn的输入对应不上了，
            encoder因为拼接了fw和bw变成了200，有2个解决办法，将encoder rnn的num_units变成decoder的一半
            或者反过来将decoder rnn的num_units增大一倍
            '''
            encoder_outpus = tf.concat(bi_outputs, -1)
            '''
            bi_state同样有fw和bw，怎样拼接在一起呢？
            (1)参照output的拼接直接，拼接c和h的最后一个维度：这种方法问题在于tuple必须是特殊类型的tuple,
            比如LSTMStateTuple
            (2)直接将fw和bw的结果堆叠在一起这样cell的个数相当于翻了一倍，需要调整encoder或者decoder的cell
            个数另外使用不同的cell也有区别的，LSTM有c和h，而GRU只有一个值。
            '''
    ...
    def build_decoder(self, encoder_states,
        ...
        with tf.name_scope("decoder_rnn"):
            self.decoder_rnn_layer = self.get_rnn_layer(decoder_rnn_layer_size,
                                                        decoder_num_units,
                                                        decoder_cell_type)
        ...
        with tf.name_scope("decoder_porjection"):
            # 这里projection_layer不指定激活函数为softmax，最后构建loss的传入的是logits，
            # logits是没有经过激活函数的值，logits = W*X+b
            self.projection_layer = tf.layers.Dense(
                                    units=self.tgt_vocab_size,
                                    kernel_initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.1))
        # 训练使用的training_helper
        training_helper = tf.contrib.seq2seq.TrainingHelper(self.embedded_tgt_batch_x,
                                                            self.tgt_batch_seq_len,
                                                            time_major=False)
        decoder = tf.contrib.seq2seq.BasicDecoder(self.decoder_rnn_layer,
                                                  training_helper,
                                                  encoder_states,
    ...
    def inference_greedy(self, encoder_states):
        ...
        #tf.tile将常量重复shape次数连接在一起
        start_tokens = tf.tile(tf.constant(value=self.start_token_id, dtype=tf.int32,shape=[1]),
        multiples = inuput_batch, name="start_tokens")
        '''
        predicting_helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(self.tgt_embedding,
                                                                     self.start_tokens,
                                                                     self.end_token_id)
        predicting_decoder = tf.contrib.seq2seq.BasicDecoder(self.decoder_rnn_layer,
                                                             predicting_helper,
                                                             encoder_states,
                                                             output_layer=self.projection_layer)
    ...
    def inference_beam_search(self, beam_width, encoder_states):
        ...
        # predicted_ids 保存了所有结果，从好到坏排名，这里直接取最好的
        tf.identity(predicting_decoder_output.predicted_ids[:, :, 0], name='predictions')
    
    def seq_loss(self, rnn_output):
        batch_size = 128
        cost = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=rnn_output, labels=self.tgt_batch_y)
    ...
    # 构造计算图，包括loss，优化等
    def build_graph(self, src_embedding_size,
                    tgt_embedding_size,
        ...
        with train_graph.as_default():
            ...
            if is_encoder_bidirectional:
                # 需要注意使用bidirectional，encoder rnn的num_units变为decoder的一半，这是为了能够保证
                # encoder_states和decoder的输入shape能对应上
                encoder_outputs, encoder_states = self.build_encoder_bi(rnn_layer_size,
                                                                        rnn_num_units/2, cell_type)
            else:
    ...
    def train(self, model_path,
        ...
        with tf.Session(graph=train_graph) as sess:
            ...
            # 获取数据生成器
            for src_batch, tgt_batch_x, tgt_batch_y, tgt_batch_max_len,\
                src_batch_seq_len, tgt_batch_seq_len, batch_num, epoch_num in generator:
