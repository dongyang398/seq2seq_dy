...
class DataInfo(object):
    def __init__(self, source_max_len,
                 source_token_list,
                 source_token_2_id,
                 target_max_len,
                 target_token_list,
...
def preprocessing(source_seq_list,
                  target_seq_list,
    ...
    print(source_token_2_id)
    print(target_token_2_id)
    ...
    dataInfoObj = DataInfo(source_max_len,
                           source_token_list,
                           source_token_2_id,
                           target_max_len,
                           target_token_list,
...
def batch_generator(source_seq_list,
                    target_seq_list,
                    dataInfoObj,
                    batch_size=128,
                    epochs=20):
    # 将seq中的字符变成int，并补充<pad> <s> </s>
    source_seq_int, source_seq_len_real = source_seq_list_2_ids(dataInfoObj,
                                                                source_seq_list)
    target_seq_int, target_seq_len_real = target_seq_list_2_ids(dataInfoObj,
                                                               target_seq_list)
    print('source_seq_int', source_seq_int)
    print('source_seq_len_real', source_seq_len_real)
    ...
    for i in range(epochs):
        ...
        for j in range(num_batch):
            ...
            source_batch_seq_len = [source_seq_len_real[index]
                                    for index in indices[j*batch_size:(j+1)*batch_size]]
            ...
            target_batch_seq_len = [target_seq_len_real[index]
                                    for index in indices[j*batch_size:(j+1)*batch_size]]
            '''
                    x:<s> A B C D  <pad> <pad>
                    y:A  B C D </s> <pad> <pad>
            ...
            target_batch_y = [[target_seq_int[index, k] for k in range(1, target_seq_len_real[index])]
                              for index in indices[j*batch_size:(j+1)*batch_size]]
            ...
            yield source_batch, target_batch_x, target_batch_y, \
                  target_batch_max_len, source_batch_seq_len, target_batch_seq_len, \
                    str(j+1)+'/'+str(num_batch), str(i+1)+'/'+str(epochs)


# 将source文件和target文件加载进来，存储到2个list中
# 只需要重写make_source_target_list这个方法就可以加载其他数据
def make_source_target_list(source_path,
                            target_path,
                            source_split_char=None,
                            target_split_char=None,
                            source_encoding="gbk",
                            target_encoding="gbk"):
...
def load_data(source_data_path,
              ...
              source_encoding="gbk",
              target_encoding="gbk"):
    source_seq_list, target_seq_list = make_source_target_list(source_data_path,
                                                               target_data_path,
                                                               source_split_char,
                                                               target_split_char,
                                                               source_encoding,
                                                               target_encoding)
    dataInfoObj = preprocessing(source_seq_list,
                                target_seq_list,
                                source_minimum_word_frequency,
                                target_minimum_word_frequency)
    gen = batch_generator(source_seq_list,
                          target_seq_list,
                          dataInfoObj,
                          batch_size,
                          epochs)
